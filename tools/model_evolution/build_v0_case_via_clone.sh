...
# this when the V0 check should CONTINUE an existing diagnostic case's restart (e.g. a
# short CONTINUE_RUN segment from a completed spin-up), rather than build a fresh
# cold-start chain — for that, use build_v0_case_via_create_case.sh instead.
#
...
#       [--stop-n N] [--stop-option OPT] [--run-startdate DATE] [--continue-run TRUE|FALSE] \
...
#   --stop-n N / --stop-option OPT / --run-startdate DATE / --continue-run T|F
...
_build_clone() {
    ...
    [ -n "$CONTINUE_RUN" ]   && ./xmlchange CONTINUE_RUN="$CONTINUE_RUN"
