...
missing and is scratch there -- these outputs regenerate on every
run. For a diagnosis worth keeping as a durable record, point
--output-dir at the site's own use_cases/{site}/memory/phase_results/
{stem}/ instead (restart-failed-jobs Step 0's $TMP is the canonical
example) -- that location is git-trackable for these filenames as of
2026-08-14 (CLAUDE.md Operating Rule #3).
...
def main():
    ...
    p.add_argument('--output-dir',
                   help='Directory for output files. Default: <dir-of-restart-script>/tmp/ '
                        '(auto-created if missing, scratch). Point this at a site\'s own '
                        'use_cases/{site}/memory/phase_results/{stem}/ instead for a durable, '
                        'git-trackable record of this diagnosis.')
