...
This page lists every Fortran module (`.F90` file) in `components/elm/src/` at commit `60d9aad`, grouped by subdirectory. Each entry is a one-line description derived from the module's top-of-file `!DESCRIPTION:` comment. When no such comment exists, the description is taken from the first substantive comment line in the file, or, as a last resort, derived from the module's name and first type declaration.

**Total:** 239 files across 7 subdirectories.

| Subdirectory | Files | Subject area |
|---|---:|---|
| [`main/`](#main-63-files) | 63 | Driver, initialization/finalization, control flags, subgrid bookkeeping, history/restart I/O, coupling glue |
| [`cpl/`](#cpl-6-files) | 6 | MCT and ESMF driver entry points, import/export of coupler fields, topounit downscaling |
| [`data_types/`](#data_types-12-files) | 12 | Gridcell/topounit/landunit/column/vegetation derived types and instance containers |
| [`dyn_subgrid/`](#dyn_subgrid-17-files) | 17 | Transient land cover (pftdyn, harvest, crop), state conservation across weight changes |
...
| `elmfates_paraminterfaceMod.F90` | NOTE(bja, 2017-01) this code can not go into the main clm-fates |
...
| `lnd_comp_esmf.F90` | ESMF-based land component interface to the E3SM driver |
