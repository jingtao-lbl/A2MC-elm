...
This page describes the directory layout of `components/elm/src/` at commit `d40b8431`. The tree contains 242 Fortran modules (`.F90`), several `.F90.in` genf90 templates that regenerate at build time, one shared `CMakeLists.txt`, one `README.unit_testing` note, and the `external_models/` and `.ipynb_checkpoints/` subtrees that are excluded from this wiki.
...
├── cpl/                 ( 5 .F90)   MCT coupler entry point; field import/export; topounit downscaling
├── data_types/          (13 .F90)   Gridcell/topounit/landunit/column/vegetation derived types + MOAB
├── dyn_subgrid/         (18 .F90)   Transient land cover: pftdyn, crop, harvest, FATES + LUH2 coupling
├── external_models/     (excluded — FATES source, documented separately)
├── main/                (65 .F90)   Driver, init/final, control flags, subgrid bookkeeping, I/O
└── utils/               (13 .F90)   Time manager, SPMD, domain, orbital, namelist helpers
```

**Total** `.F90` files in scope: **242** across 7 subdirectories (up from 239 at `60d9aad`: `cpl/` lost 1 file, `data_types/` gained 1, `dyn_subgrid/` gained 1, `main/` net gained 2). In addition, `dyn_subgrid/` carries three genf90 templates (`dynVarMod.F90.in`, `dynVarTimeInterpMod.F90.in`, `dynVarTimeUninterpMod.F90.in`) plus the `do_genf90` helper, `main/` carries `ncdio_pio.F90.in`, and `utils/` carries `restUtilMod.F90.in` and a `dtypes.h` header. None of those are counted in the 242 total.

## `main/` — driver, initialization, and shared bookkeeping (65 files)
...
- **Driver and lifecycle.** `elm_driver.F90` (the `elm_drv` entry point called every timestep; `main/elm_driver.F90:207`), `elm_initializeMod.F90` (`initialize1`/`initialize2`/`initialize3`; `main/elm_initializeMod.F90:62,503,1146`), `elm_finalizeMod.F90`, `elm_instMod.F90` (instantiation of physics-state derived types), and `elm_instance.F90` (multi-instance bookkeeping).
...
- **Coupling glue.** `atm2lndMod.F90`/`atm2lndType.F90`, `lnd2atmMod.F90`/`lnd2atmType.F90`, `glc2lndMod.F90`, `lnd2glcMod.F90`, `glcDiagnosticsMod.F90`, `iac2lndMod.F90`, `lnd2iacMod.F90`, `ocn2lndType.F90`, `elmfates_interfaceMod.F90` (now the sole FATES coupling module — `elmfates_paraminterfaceMod.F90` was removed at this commit), `elm_interface_bgcType.F90`, `elm_interface_dataType.F90`, `elm_interface_thType.F90`, `elm_interface_funcsMod.F90`, `elm_interface_pflotranMod.F90`, `init_hydrology.F90`.
...
The directory also holds `findHistFields.pl` (a Perl helper) and `ncdio_pio.F90.in` (a genf90 template for the PIO I/O wrapper); neither is counted in the 65 `.F90` total.
...
- **BeTR variants** for tracer-transport coupling. When `use_betr` is true, the BeTR-aware modules (`CNAllocationBetrMod.F90`, `CNEcosystemDynBetrMod.F90`, `CNPhenologyBeTRMod.F90`, `CNGapMortalityBeTRMod.F90`, `CNNStateUpdate1BeTRMod.F90`, `CNNStateUpdate2BeTRMod.F90`, `CNNStateUpdate3BeTRMod.F90`) replace the default modules.
...
- **Crop.** `CropMod.F90`, `CropType.F90`, `CropHarvestPoolsMod.F90`, `WoodProductsMod.F90`, `FanMod.F90`, `FanUpdateMod.F90`. (Crop prognostic state such as `cropplant`/`harvdate` lives in `data_types/CNStateType.F90`.)
...
## `dyn_subgrid/` — transient land cover (18 files)
...
- **Conservation.** `dynConsBiogeochemMod.F90` (C and N conservation), `dynConsBiogeophysMod.F90` (water and energy conservation).
- **FATES coupling.** `dynEDMod.F90`, `dynFATESLandUseChangeMod.F90`. The latter is new at `d40b8431` and reads the LUH2 land-use harmonization dataset (108 transition variables, 12 state variables, 5 harvest variables) through `dyn_var_time_uninterp_type` from `dynVarTimeUninterpMod.F90`.

The directory also contains three genf90 templates (`dynVarMod.F90.in`, `dynVarTimeInterpMod.F90.in`, `dynVarTimeUninterpMod.F90.in`) and the `do_genf90` helper script; these regenerate Fortran sources at build time and are not counted in the 18 `.F90` total.

See [`dyn_subgrid/index.md`](../dyn_subgrid/index.md) for the subsystem walkthrough.

## `data_types/` — derived-type skeletons for subgrid levels (13 files)
...
In addition, `VegetationPropertiesType.F90` holds PFT-level parameters (traits, allometry, phenology), `CNStateType.F90` holds CN-specific state shared across levels (including crop prognostic variables), and **`MOABGridType.F90`** (new at `d40b8431`, 756 lines, guarded by `#ifdef HAVE_MOAB`) provides Mesh-Oriented dAtaBase support for unstructured-mesh grids. `MOABGridType.F90` sits outside the five-level subgrid hierarchy and is built only when MOAB is enabled in the build configuration.

The topounit level is ELM-specific — see `TopounitType.F90:1-7` for the documented hierarchy note.
...
- `domainMod.F90`, `domainLateralMod.F90` — surface-domain bookkeeping and lateral connectivity (PETSc-based lateral flow; see `utils/domainLateralMod.F90:1`).
...
The directory also contains `restUtilMod.F90.in` (a genf90 template for the restart utility module) and `dtypes.h` (a small C header used by the SPMD layer); neither is counted in the 13 `.F90` total.

## `cpl/` — coupler interface (5 files)

`cpl/` is ELM's interface to the external coupler. It is deliberately thin and contains only what the coupler sees. As of `d40b8431`, the historical separate ESMF entry-point file `lnd_comp_esmf.F90` no longer exists; the MCT module remains the single component entry point and continues to import `ESMF_Clock` from the ESMF Fortran types for clock handling.

- **Driver entry point.** `lnd_comp_mct.F90` (MCT path; see `cpl/lnd_comp_mct.F90:58,437,684` for init/run/final).
...
- **`external_models/`** — FATES source code. Documented in the separate FATES wiki (paired with FATES commit `e027a40`, sci.1.91.1_api.43.1.0).
