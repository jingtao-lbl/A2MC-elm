...
- `dayl_patch` from `DaylengthMod` is used by CN/CNP phenology in `biogeochem/PhenologyMod.F90` (and the BeTR-coupled variant `biogeochem/CNPhenologyBeTRMod.F90`) and A2MC's FATES phenology calibration workflow (see `use_cases/Kougarok/memory/gained_knowledge/discoveries.json`).
