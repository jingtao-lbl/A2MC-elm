...
MESSAGE = (
    ...
    "missed once:\n"
    "  1. THREE LAYERS, not two: (a) nohup the watcher script; (b) arm `Monitor` on its LOG "
    "-- a poll loop run directly as the Monitor command is session-local (anti-pattern #5); "
    "(c) arm a THIRD Monitor that queries `squeue` DIRECTLY (Step 8), depending on no watcher "
    "script and no log file. Layers 1-2 both go silent if the watcher dies or was never armed "
    "for a case, and a dependency zombie is invisible to them; layer 3 is what still sees it.\n"
