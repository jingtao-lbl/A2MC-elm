...
> **Why this matters.** The curated KB shapes the AI's diagnoses. The May 2026
> contamination (`memory/dev_logs/20260519a_*`) was 70 false `auto_discovered` entries
> — including `do_not_repeat` rules that **forbade the AI's own best fixes**. The write
> gate (`20260612d`) stops those reaching curated knowledge automatically; **but if no
...
- The staging file `auto_discovered_pending.json` is gitignored run-state; promoted
  ...
  curation policy.
- Pairs with: the write gate `tools/review_pending_knowledge.py` /
