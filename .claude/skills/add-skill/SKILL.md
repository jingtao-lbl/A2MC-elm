...
2. **Name + scope.** Pick a kebab-case `<name>` (A2MC skills are **unprefixed** —
   ...
   **Branch-scoped:** this is `kougarok_fates_demo` (api-31-0, manuscript flow) — a skill
   that only makes sense for `main`/adapter-kit doesn't belong here. Note the branch fit in
   the skill's Notes.
