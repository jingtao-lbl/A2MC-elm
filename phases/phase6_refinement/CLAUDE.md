...
| Script | Purpose |
|--------|---------|
| `evaluate_results.py` | Evaluate experiments vs hypotheses, extract lessons, determine next action (extracted from `orchestrator.py`) |
| `plot_experiment_comparison.py` | Experiment-vs-baseline biomass panel figure with validation targets |

(Knowledge persistence itself runs through `tools/extract_knowledge.py` + `memory/manager.py`, not a phase-local script.)
