#!/usr/bin/env python3
"""
Generate FATES Parameter Files from Sampling Ensemble (method-agnostic).

Reads a sampling ensemble matrix (Morris / Sobol / LHS — produced by
phases/phase0_design/create_parameter_sample.py) and materializes one
FATES parameter file per ensemble case.

Format is determined by the template parameter file's extension:
    .json -> JSON output (api-43+ FATES; canonical milestone)
    .nc   -> NetCDF output (legacy api-31 milestones)

Parameter modifications go through tools/modify_fates_parameters.py,
which dispatches on format internally — this script does not need to
know about JSON vs NetCDF specifics.

Key features:
1. Handles PFT-specific (1D) parameters
...
History:
    - 2025-12-30 Created (NetCDF-only)
    - 2026-05-12 Renamed from create_morris_ensemble.py + format-agnostic
"""
...
from tools.modify_fates_parameters import create_modified_parameter_file, detect_format
...
def _apply_post_modifications_json(output_file, param_values):
    ...
    n_pft = len(params["fates_cnp_prescribed_puptake"]["data"])
    params["fates_cnp_prescribed_puptake"]["data"] = [0.0] * n_pft
    params["fates_cnp_prescribed_nuptake"]["data"] = [0.0] * n_pft
...
def main():
    ...
    # v2.100: detect base file format; output files take the same extension.
    # api-43+ -> .json; api-31 legacy -> .nc.
    base_fmt = detect_format(BASE_PARAM_FILE)
    out_ext = ".json" if base_fmt == "json" else ".nc"
    print(f"Base file format: {base_fmt} → output extension: {out_ext}")
    ...
    for i in range(n_sets):
        ...
        # Output filename (extension matches base file format)
        output_file = OUTPUT_DIR / f"fates_params_api25.5.0_12pft_c230710__PtCNP162_En{set_id}{out_ext}"

        # Create parameter file (format-dispatching via the public API)
    ...
    # Verification
    print(f"\n--- Verification ---")
    n_files = len(list(OUTPUT_DIR.glob(f"*{out_ext}")))
    ...
    # Sample verification: check first and last file (format-aware)
    print(f"\n--- Sample Verification (first and last files) ---")
    PFT7, PFT9, PFT10 = 6, 8, 9  # 0-based indices
    for check_id in [1, n_sets]:
        check_file = OUTPUT_DIR / f"fates_params_api25.5.0_12pft_c230710__PtCNP162_En{check_id}{out_ext}"
        if not check_file.exists():
            continue
        if base_fmt == "json":
            with open(check_file) as f:
                jdoc = json.load(f)
            params = jdoc["parameters"]
            val1 = float(params['fates_cnp_eca_alpha_ptase']['data'][PFT7])
            val2 = float(params['fates_alloc_storage_cushion']['data'][PFT10])
            val3 = float(params['fates_cnp_prescribed_puptake']['data'][0])
        else:
