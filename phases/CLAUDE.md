...
The Phase 0→7 workflow below is the **single methodology both A2MC agent modes follow** — it is the calibration roadmap, not an implementation detail of the orchestrator:

- The **autonomous (online) agent** (`orchestrator.py --run`) *traverses* the phases mechanically as a fixed state machine.
- The **interactive (offline) agent** (a coding-agent harness driven by conversation) *navigates* the same phases with judgment — it enters at the right phase for the task (see **Entry Points** below), applies that phase's discipline, and uses the same nested loops (see **Three-Level Iteration Structure** below). It is phase-aware and iterative, not rigidly sequential.
...
The phases run as **three nested loops**, not a single pass. Both agent modes use the same structure (the autonomous agent tracks it with explicit counters; the interactive agent navigates it by judgment):
